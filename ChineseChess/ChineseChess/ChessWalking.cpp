...
int ChessWalking::walk(int id, int x, int y, int& previousCursonX, int &previousCursonY, wchar_t text, Chess temp, int powx,int powy)
{
	Board temp1;
	hin = GetStdHandle(STD_OUTPUT_HANDLE);
	...
	{
	if (powx == x && powy == y)return 1;
	else if (Board::CurrentBoard[Board::ConvertToBoardPoint().y][Board::ConvertToBoardPoint().x].GetID() != 0 && (powx != x || powy != y))return 0;
		if ( (x != -1) && (y != -1))
		{
			int hasAObstacleX = x;
			int hasAObstacleY = y;
			...
			for (int x = 0; x <= 8; x++)
			{
				for (int y = 0; y <= 10; y++)
				{
					if ((x >= 3 && x <= 5) && (y != -1) && y <= 2 && (abs(x - previousCursonX) + abs(y - previousCursonY) <= 1))
					{
						...
						else  if (Board::CurrentBoard[y][x].GetID() != 0)
						{
